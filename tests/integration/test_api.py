...
def test_ready(client: httpx2.Client) -> None:
    resp = client.get("/ready")
    assert resp.status_code == 200


def test_health(client: httpx2.Client) -> None:
...
def test_formats_match_catalog(client: httpx2.Client) -> None:
...
def test_options_epub_to_output(client: httpx2.Client, out_fmt: str) -> None:
...
def test_options_invalid_input_format(client: httpx2.Client) -> None:
    resp = client.get("/formats/bogus/epub/options")
    assert resp.status_code == 404


def test_options_invalid_output_format(client: httpx2.Client) -> None:
